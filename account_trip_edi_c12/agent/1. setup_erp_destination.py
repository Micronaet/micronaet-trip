...
file_xlsx = './data/destinazioni.xlsx'
...
try:
    ...
    print('[ERROR] Cannot read XLSX file: %s' % file_xlsx)
    sys.exit()
...
for row in range(from_row, WS.nrows):
    master_code = WS.cell(row, 1).value
    child_code = WS.cell(row, 1).value

    ref = 'SER%s-%s' % (master_code, child_code)
    ...
    data = {}
    if partner_ids:
        print('Updating %s' % ref)
        partner_pool.write(partner_ids, data)
    else:
        print('Creating %s' % ref)
